                ...
                <nav class="flex items-center justify-between h-16">
                    <div class="flex items-center gap-8">
                        ...
                        <div class="hidden md:flex items-center gap-6 text-sm font-medium">
                            <a href="{{ route('books.index') }}" class="hover:text-[#f53003] transition-colors">Books</a>
                            @auth
                                @if(Auth::user()->role === 'client')
                                    <a href="{{ route('client.dashboard') }}" class="hover:text-[#f53003] transition-colors">My Dashboard</a>
                                @endif
                                @if(in_array(Auth::user()->role, ['admin', 'employee']))
                                    <a href="{{ route('admin.dashboard') }}" class="hover:text-[#f53003] transition-colors">Admin</a>
                                @endif
                            @endauth
                        </div>
                    </div>
